...
def _slug_from_host(host: str) -> str:
    value = "".join(ch if ch.isalnum() else "-" for ch in str(host or "").strip().lower())
    value = value.strip("-")
    return value or "audit"


class SignalAtlasStorage:
    def __init__(self, root: Optional[Path] = None) -> None:
        self.root = Path(root or (get_runtime_root() / "signalatlas"))
    ...
    def _load_index_locked(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"[SIGNALATLAS] Audit index ignored ({exc})")
        return {"version": 1, "audits": {}}
    ...
    def get_audit(self, audit_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[SIGNALATLAS] Audit read failed for {audit_id}: {exc}")
            return None
    ...
    def save_audit(self, audit: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            index.setdefault("audits", {})[audit_id] = {
                "id": audit_id,
